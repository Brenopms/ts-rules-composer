...
describe('match combinator', () => {
  ...
  const mockFail = vi.fn().mockResolvedValue({ status: 'failed', error: 'error' });
  ...
  it('should maintain type safety with complex inputs', async () => {
    ...
    const complexRule: Rule<ComplexInput> = () => Promise.resolve({ status: 'passed' });
    
    const matcher = match(
      (input: ComplexInput) => input.nested.value,
      {
        test: complexRule,
      }
    );
